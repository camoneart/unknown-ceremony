このルールファイルは、Next.jsのプロジェクト開発時に必ず必ず必ず必ず発動。

## リファレンス

`.claude/rules/nextjs-bestpractice-rule/docs/`

リファレンスの設計思想に必ず必ず必ず必ず従うこと。

このルールファイルの各セクションで、URLパスを指定してる箇所が複数あるが、全て `.claude/rules/nextjs-bestpractice-rule/docs/` の中にあるので必ず必ず必ず必ず参照すること。
...
- `.claude/rules/nextjs-bestpractice-rule/docs/` - ドキュメントベース（Biomeチェック対象外）
- ルートレベルの設定ファイル
...
## テスト戦略
- テストはコンポーネントと同じ場所に配置
- テストは`.claude/rules/nextjs-bestpractice-rule/docs/testing/frontend-unit-testing.md`を必ず必ず必ず必ず参照し、AAAパターンで記述
...
`.claude/rules/nextjs-bestpractice-rule/docs/`ディレクトリには、React・Next.js・テストに関する包括的なベストプラクティスドキュメントが含まれています。

### 主要ドキュメント構成

#### 1. Next.js基本原理ガイド (`.claude/rules/nextjs-bestpractice-rule/docs/nextjs-basic-principle/`)
...
#### 2. 単体テスト (`.claude/rules/nextjs-bestpractice-rule/docs/testing/`)
**フロントエンド単体テスト** (`.claude/rules/nextjs-bestpractice-rule/docs/testing/frontend-unit-testing.md`)
