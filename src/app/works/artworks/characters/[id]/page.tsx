...
const CharacterDetailPage = async ({ params }: Props) => {
  const { id } = await params;
  const character = artworks.find((art) => art.id === id);
      ...
      <div className="min-w-[750px] max-w-[750px] grid gap-[20px] pb-[50px]">
        ...
        <div className="grid gap-[10px]">
          <div className="border-b border-white/50 pb-[10px]">
            <h1 className="text-[20px] px-[5px] text-center">{character.title}</h1>
          </div>
          <p className="px-[5px] leading-[1.75]">{character.description}</p>
        </div>
      </div>
    </div>
  );
